...
function BookTestModal({ selectedTests, allTests, onClose, onBooked }) {
  ...
  function handleBook() {
    ...
    try {
      const existing = JSON.parse(localStorage.getItem("mm_bookings") || "[]");
      existing.unshift(booking);
      localStorage.setItem("mm_bookings", JSON.stringify(existing));
    } catch {}
    onBooked(booking);
    setStep("success");
  }
      ...
      <div style={{ width:"100%",maxWidth:500,background:"#0D1018",border:"1px solid "+C.border,borderRadius:"22px 22px 0 0",padding:"28px 22px 36px",animation:"fadeUp 0.28s ease" }}>
        {step === "form" ? (
          <>
            ...
            <button onClick={handleBook} style={{ ...F,width:"100%",padding:"14px",background:"linear-gradient(135deg,#00D4AA,#00956E)",border:"none",borderRadius:12,color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer",boxShadow:"0 4px 18px rgba(0,212,170,0.3)" }}>
              Confirm Booking →
            </button>
          </>
        ) : (
...
export default function MedMatch() {
  ...
  const [tests, setTests] = useState(() => {
    try { const s = localStorage.getItem("mm_tests"); return s ? JSON.parse(s) : DEFAULT_TESTS; }
    catch { return DEFAULT_TESTS; }
  });

  const [bookings, setBookings] = useState(() => {
    try { return JSON.parse(localStorage.getItem("mm_bookings") || "[]"); }
    catch { return []; }
  });

  // Refresh bookings from localStorage when admin opens
  useEffect(() => {
    if (adminAuthed && adminTab === "bookings") {
      try { setBookings(JSON.parse(localStorage.getItem("mm_bookings") || "[]")); }
      catch {}
    }
  }, [adminAuthed, adminTab]);

  const saveDepts = d => { setDepts(d); try { localStorage.setItem("mm_depts",JSON.stringify(d)); } catch {} };
  const saveTests = t => { setTests(t); try { localStorage.setItem("mm_tests",JSON.stringify(t)); } catch {} };
  ...
  function updateBookingStatus(id, status) {
    try {
      ...
      localStorage.setItem("mm_bookings", JSON.stringify(updated));
      setBookings(updated);
    } catch {}
            ...
            <>
              ...
              {adminTab === "tests" && (
                <>
                  ...
                  {tests.map(t => (
                    <div key={t.id} className="test-row" style={{ background:C.card,border:"1px solid "+C.border,borderRadius:11,padding:"12px 15px",marginBottom:7,display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,transition:"border-color 0.15s" }}>
                      ...
                      <button onClick={()=>removeTest(t.id)} style={{ ...F,background:C.redDim,border:"1px solid "+C.red+"30",color:C.red,borderRadius:7,padding:"5px 10px",fontSize:12,cursor:"pointer",fontWeight:600,flexShrink:0 }}>Remove</button>
                    </div>
                  ))}
                </>
              )}
              ...
              {adminTab === "bookings" && (
                <>
                  <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:13 }}>
                    ...
                    <button onClick={()=>{ try{setBookings(JSON.parse(localStorage.getItem("mm_bookings")||"[]"));}catch{} }}
