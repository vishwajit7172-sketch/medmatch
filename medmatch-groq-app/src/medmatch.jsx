...
function TestsSection({ tests }) {
  ...
  // Smart filter: search takes priority over category tabs
  const filtered = (() => {
    ...
    return tests.filter(t => {
      ...
      // 1. Name starts with query (e.g. "E" → ECG, Echocardiogram)
      if (name.startsWith(q)) return true;
      // 2. Name contains query
      if (name.includes(q)) return true;
      // 3. Category matches query
      if (cat.includes(q)) return true;
      // 4. Dept/specialty keyword match (e.g. "cardiology" → ECG, Echocardiogram, Lipid Profile)
    ...
    <div>
      ...
      <div style={{ position:"relative",marginBottom:14 }}>
        ...
        <input
          ...
          placeholder="Search tests or specialty (e.g. E, cardiology…)"
          style={{ ...F,width:"100%",padding:"10px 36px 10px 38px",background:"rgba(13,18,30,0.85)",border:"1px solid "+(searchQuery?C.accent+"60":C.border),borderRadius:11,fontSize:13,color:C.text,outline:"none",boxSizing:"border-box",transition:"border-color 0.18s" }}
        />
        {searchQuery && (
          <button onClick={() => setSearchQuery("")}
            style={{ position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:C.muted,fontSize:17,cursor:"pointer",lineHeight:1,padding:"2px 5px" }}>×</button>
        )}
      </div>

      {/* Category filter tabs — hidden when searching */}
      {!searchQuery && (
      ...
      {/* Search result label */}
      {searchQuery && (
        <div style={{ fontSize:12,color:C.muted,marginBottom:12,paddingLeft:2 }}>
          {filtered.length > 0
            ? <span><span style={{ color:C.accentText,fontWeight:600 }}>{filtered.length}</span> result{filtered.length!==1?"s":""} for "<span style={{ color:C.text }}>{searchQuery}</span>"</span>
            : <span style={{ color:C.muted }}>No tests found for "<span style={{ color:C.red }}>{searchQuery}</span>"</span>
          }
        </div>
      )}
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:8,marginBottom:16 }}>
        {filtered.length === 0 && searchQuery && (
          <div style={{ textAlign:"center",padding:"36px 20px",background:C.card,border:"1px solid "+C.border,borderRadius:14 }}>
            <div style={{ fontSize:34,marginBottom:10 }}>🔬</div>
            <div style={{ fontSize:14,color:C.text,fontWeight:600,marginBottom:6 }}>No tests found</div>
            <div style={{ fontSize:12,color:C.muted }}>Try a different name or specialty like "cardiology", "blood", "radiology"</div>
          </div>
        )}
        {filtered.map(t => {
...
export default function MedMatch() {
  ...
  // Push a dummy history entry whenever we leave home so the OS back gesture
  // pops it instead of closing the app.
  useEffect(() => {
    if (screen !== "home") {
      window.history.pushState({ medmatch: true }, "");
    }
