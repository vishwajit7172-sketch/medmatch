...
const F  = { fontFamily: "'Outfit', 'DM Sans', system-ui, sans-serif", fontStretch: "normal" };
const FB = { fontFamily: "'DM Sans', system-ui, sans-serif", fontStretch: "normal" };
...
function BookTestModal({ selectedTests, allTests, onClose, onBooked }) {
  ...
  return (
    <div style={{ position:"fixed",inset:0,zIndex:5000,background:"rgba(11,24,38,0.55)",backdropFilter:"blur(10px)",display:"flex",alignItems:"flex-end",justifyContent:"center" }} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{ width:"100%",maxWidth:500,background:C.card,borderRadius:"28px 28px 0 0",padding:"28px 22px 40px",animation:"slideUp 0.3s cubic-bezier(0.34,1.56,0.64,1)",boxShadow:"0 -20px 60px rgba(11,24,38,0.2)",maxHeight:"90vh",overflowY:"auto" }}>
        {step === "form" ? (
          <>
  ...
  return (
    <div>
      ...
      {selectedTests.length > 0 && (
        <div style={{ position:"sticky",bottom:16,background:C.navy,borderRadius:20,padding:"16px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,boxShadow:"0 8px 40px rgba(11,24,38,0.35)",zIndex:10 }}>
          <div>
...
export default function MedMatch() {
  ...
  return (
    <div style={{ ...FB, minHeight:"100vh", background:C.bg, color:C.text, position:"relative" }}>
      ...
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@600;700;800&family=DM+Sans:opsz,wght@9..40,400;9..40,500;9..40,600;9..40,700&display=swap');
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .admin-btn{width:46px;height:46px;border-radius:50%;background:${C.navy};border:none;color:#fff;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 24px rgba(11,24,38,0.3);transition:all 0.2s;font-family:'DM Sans',system-ui;font-stretch:normal;}
